...
class Router {
    ...
    static getRoutes(applicationName) {
        var routesDefinition = require('../applications/' + applicationName + '/router.json');
    ...
    static _processRoutesConfig(definition, applicationName) {
        ...
        builtRoute['component'] = require('../applications/' + applicationName + '/components/' + definition['component']);
        ...
        if (definition.hasOwnProperty('childRoutes')) {
            var childRoutes = [];
            for (var singleChild of definition['childRoutes']) {
